                ...
                <div class="modal fade" id="defaultModalPrimary" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('tender_sub_category.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="modal-body m-3">
                                    <div class="for-group">
                                        <label>Tender Sub Category Name</label>
                                        <input type="text" name="name" class="form-control" placeholder="category name">
                                    </div>
                                    <br>
                                    <div class="for-group">
                                        <label>Tender sub Category Name (Amharic)</label>
                                        <input type="text" name="name_am" class="form-control" placeholder="category name">
                                    </div>
                                    <br>
                                    <label for="my-select">Select Category</label>
                                    <select id="my-select" class="form-control" name="tender_category_id" required>>
                    ...
                    <table class="table mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="row">
                                        <button class="btn btn-primary mr-2" data-toggle="modal" data-target="#defaultModalEdit{{ $posts->id }}">Edit
                                        </button>
                                        <form method="POST" action="{{ route('tender_sub_category.destroy', $posts->id)}}">
                                            @csrf 
                                            @method('DELETE')
                            ...
                            <div class="modal fade" id="defaultModalEdit{{$categories->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">Edit Tender Sub Category</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <div class="modal-body m-3">
                                            <form action="{{ route('tender_sub_category.update', $categories->id)}}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PATCH')
                                                <div class="modal-body m-3">
                                                    <div class="for-group">
                                                    ...
                                                    <div class="for-group">
                                                        <label>Tender Sub Category Name (Amharic)</label>
                                                        <input type="text" name="name_am" class="form-control" value="{{$categories->name_am}}" placeholder="category name">
                                                    </div>
                                                    <br>
                                                    <label for="my-select">Select Category</label>
                                                    <select id="my-select" class="form-control" name="tender_category_id">
                                                        @if(App\TenderCategory::all()->count() > 0)
                                                            @foreach(App\TenderCategory::all() ->sortBy('name') as $categories)
                                                            <option value="{{$categories->id}}" required>{{$categories->name}}</option>
                                                            @endforeach
